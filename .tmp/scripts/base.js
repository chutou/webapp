...
function getByClass(id, className) {
  ...
  if (ct.getElementsByClassName) {
    ...
    for (var i = 0, l = item.length; i < l; i++) {
      if (reg.test(item[i].className)) result.push(item[i]);
    }
    return result;
  }
}
...
Fader.prototype = {
  auto: function auto() {
    ...
    function autoLoop() {
      ...
      that.ct.onmouseover = function () {
        clearTimeout(lp);
      };
      that.ct.onmouseout = function () {
        lp = setTimeout(autoLoop, 3000);
      };
    };
    autoLoop();
  },
  click: function click(next) {
    if (next != this.cur) {
      this.animate(this.cur, next);
    }
  },
  animate: function animate(cur, next) {
...
function ft(id, top) {
  ...
  addEvent(w, "scroll", function () {
    ...
    timer = setInterval(function () {
      var temp = d.documentElement.scrollTop || d.body.scrollTop,
          c;
...
//imgZoom
var imgZoom = function imgZoom(id, options) {
  var ct = this.ct = document.getElementById(id),
...
imgZoom.prototype = {
  init: function init() {
    ...
    var load = function load(a) {
      ...
      addEvent(o.prev, "click", function () {
        o.tMove(1);
      });
      addEvent(o.next, "click", function () {
        o.tMove(-1);
      });
      o.createView.call(o);
      addEvent(o.img, "mousemove", function (e) {
        o.move(e);
      });
      if (!o.options.viewer) {
        addEvent(o.img, "mouseover", function () {
          o.viewer.style.display = "";o.img.style.cursor = "crosshair";
        });
        addEvent(o.img, "mouseout", function () {
          o.viewer.style.display = "none";o.img.style.cursor = "default";
        });
      }
      for (var i = 0, l = o.thumb.length; i < l; i++) {
        (function (arg) {
          addEvent(o.thumb[arg], "click", function () {
    ...
    } else if (document.readyState == "complete") load();else addEvent(window, "load", load);
  },
  ...
  createView: function createView() {
    ...
    if (this.options.viewerPos.v < 0) {
      t -= _is.h * this.options.viewerMul;
    ...
    if (this.options.viewerPos.h < 0) {
      l -= _is.w * this.options.viewerMul;
...
var setAmount = {
  ...
  getVal: function getVal(obj) {
    return document.getElementById(obj).value;
  },
  setVal: function setVal(obj, x) {
    return document.getElementById(obj).value = x;
  },
  amount: function amount(obj, mode) {
...
var stdNav = {
  ...
  spanClick: function spanClick(obj) {
...
//调整字体大小
var act = {
  modify: function modify(id, size) {
